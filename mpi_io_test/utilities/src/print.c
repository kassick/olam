...
******************************************************************/
  if(my_rank == 0){
    ...
    fprintf(ofptr,"Time to wait between consecutive I/O Operations: %uus\n", 
                params->wait_btwn_wr);
    fprintf(ofptr,"Number of iteractions for busy-wait between I/O Ops: %u\n",
                params->busy_wait_interactions);
    fprintf(ofptr,"Wait a random amount of seconds between writing two objects?: %u\n",params->random_wait_btwn_wr);
#endif
    fflush(ofptr);
