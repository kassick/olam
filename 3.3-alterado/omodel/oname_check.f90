...
use max_dims,    only: nzgmax, maxgrds, maxsndg, maxsclr, maxnplt, maxisdirs, &
                       maxpltfiles
...
integer      :: nfatal, nwarn, ifm, ng, k, iplt, i_huge
...
if (nl%mdomain < 2) then
   do ng=1, nl%ngrids
      call rchk_bnds( nl%centlat(ng), "CENTLAT",  -90.,  90., 0, nfatal, nwarn )
      call rchk_bnds( nl%centlat(ng), "CENTLON", -180., 180., 0, nfatal, nwarn )
   enddo
endif

do ng=1, nl%ngrids
   call rchk_bnds(nl%grdlen(ng),  "GRDLEN", dzxmin, erad*pi1, 0, nfatal, nwarn )
   call rchk_bnds(nl%grdwid(ng),  "GRDWID", dzxmin, nl%grdlen(ng), 0, nfatal, nfatal, &
        msgmax="GRDWID must be <= GRDLEN for each nest")
   call rchk_bnds(nl%grdaxis(ng),"GRDAXIS",  -90.0,     90.0, 0, nfatal, nwarn )
enddo
...
if (nl%initial == 2) then
   ...
   if (nl%nudflag == 1) then
      ...
      call ichk_bnds( nl%nudrad, "NUDRAD", 1, 10, 2, nfatal, nwarn )
...
call ichk_bnds( nl%naddsc,     "NADDSC", 0, maxsclr, 0, nfatal, nwarn, &
     msgmax="Increase maxsclr in max_dims.f90 if more scalars are needed." )

call ichk_bnds( nl%icorflg, "ICORFLG", 0, 1, 0, nfatal, nwarn )

call rchk_bnds( nl%cnum_vel,   "CNUM_VEL", 0.0, 1.0, 0, nfatal, nwarn )
call rchk_bnds( nl%cnum_sclr, "CNUM_SCLR", 0.0, 1.0, 0, nfatal, nwarn )
...
call ichk_bnds( nl%iswrtyp, "ISWRTYP", 0,  4, 0, nfatal, nwarn )
call ichk_bnds( nl%ilwrtyp, "ILWRTYP", 0,  4, 0, nfatal, nwarn )
call rchk_bnds( nl%radfrq,   "RADFRQ", nl%dtlong, r_huge, 2, nfatal, nwarn )
call ichk_bnds( nl%lonrad,   "LONRAD", 0,  1, 0, nfatal, nwarn )
...
elseif (nl%level == 3) then
   ...
   if (.not. any(nl%icloud == (/0, 1, 4, 5, 7/) )) then
      write(io6,*) 'FATAL - icloud must be set to 0, 1, 4, 5, or 7.'
      nfatal = nfatal + 1
   endif
   ...
   if (nl%irain == 2) &
        call rchk_bnds( nl%rparm, "RPARM", 0., 1.e-1, 0, nfatal, nwarn )

   if (nl%isnow == 2) &
        call rchk_bnds( nl%sparm, "SPARM", 0., 1.e-1, 0, nfatal, nwarn )

   if (nl%iaggr == 2) &
        call rchk_bnds( nl%aparm, "APARM", 0., 1.e-1, 0, nfatal, nwarn )

   if (nl%igraup == 2) &
        call rchk_bnds( nl%gparm, "GPARM", 0., 1.e-1, 0, nfatal, nwarn )

   if (nl%ihail == 2) &
        call rchk_bnds( nl%hparm, "HPARM", 0., 1.e-1, 0, nfatal, nwarn )
...
if (nl%isfcl == 1) then
   ...
   if (nl%ndviflg == 2) then
      if (nl%iupdndvi == 1) then
         write(*,*) "Turning off NDVI update for ndviflg == 2"
         nl%iupdndvi = 0
         nwarn = nwarn + 1
      endif
   endif
   ...
   if (nl%isstflg == 2) then
      if (nl%iupdsst == 1) then
         write(*,*) "Turning off SST update for isstflg == 2"
         nl%iupdsst = 0
         nwarn = nwarn + 1
      endif
   endif
   ...
   if (nl%iseaiceflg == 2) then
      if (nl%iupdseaice == 1) then
         write(*,*) "Turning off SEAICE update for iseaiceflg == 2"
         nl%iupdseaice = 0
         nwarn = nwarn + 1
      endif
   endif
...
do iplt = 1, nl%nplt
   if ((nl%projectn(iplt) == 'V'  .or.   &
        nl%projectn(iplt) == 'Z') .and.  &
        nl%mdomain < 2) then
   ...
   if ((nl%projectn(iplt) == 'L'  .or.   &
        nl%projectn(iplt) == 'P'  .or.   &
        nl%projectn(iplt) == 'O'  .or.   &
        nl%projectn(iplt) == 'C') .and.  &
        nl%mdomain > 1) then
...
! ALLOWABLE VALUES OF NXP IN GLOBAL RUN WITH POLYGON NUDGING (SEVERITY - F)

if (nl%nudflag > 0 .and. nl%nudrad > 0) then
   if (mod(nl%nxp,3*nl%nudrad) /= 0) then
      write(io6,*) 'FATAL - NXP must be an integer multiple of 3*NUDRAD'
      write(io6,*) 'if polygon nudging is being done (i.e., if NUDFLAG > 0) '
      nfatal = nfatal + 1
   endif
endif
...
! MAKE SURE THAT RADIATION SCHEMES ACTIVATED IF SOIL MODEL USED. (SEVERITY - F)

if (nl%isfcl > 0 .and. nl%ilwrtyp+nl%iswrtyp == 0) then
   write(io6,*) ' FATAL  - radiation scheme must be run with soil model.'
   nfatal = nfatal + 1
endif
