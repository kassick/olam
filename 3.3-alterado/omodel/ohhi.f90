...
use micro_coms,  only: level
use mem_ijtabs,  only: jtab_w, jtab_u, itab_w, itab_u
...
use consts_coms, only: cvocp, p00k, rdry, rvap, p00, rocp, alvlocp,  &
                       gravo2, erad
use mem_grid,    only: mza, dzt, mua, lpu, unx, uny, unz, xeu, yeu, zeu,  &
                       aru, volt, volui
...
!!!!!!!!!!!!!!!!!! SPECIAL FOR L DOMAIN

!         if (xeu(iu) < 0.) then
!            uv01dx = 10.
!            uv01dy = 0.
!         elseif (yeu(iu) > 0.) then
!            uv01dx = 0.
!            uv01dy = 10.
!         else
!            uv01dx = 10. - .0005 * xeu(iu)
!            uv01dy = 10. + .0005 * yeu(iu)
!         endif

!         uc(k,iu) = uv01dx * unx(iu) + uv01dy * uny(iu)
