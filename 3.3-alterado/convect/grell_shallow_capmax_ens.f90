...
subroutine cuparth_shal(iw, dtime)
  ...
  use mem_ijtabs, only: jtab_w, istp, itab_w
  use mem_grid, only: mwa, mza, lpw, volt, zm, zt, arw0
...
subroutine CUP_enss_shal(k2, iact_gr, z1, psur, tkmin, dtime, t,   &
  ...
  real, dimension(k2), intent(in) :: qo
...
!--- calculate moist static energy, heights, qes
        call cup_env(k2, xt, po, tcrit, 2, z1, psur, xq, xhe, xhes, xqes, xz) 
...
subroutine cup_forcing_ens_shal(ensdim, k2, aa0, aa1, xaa0, mbdt,   &
  ...
  real, intent(out) :: xmb
  integer, intent(in) :: ierr
  real, dimension(ensdim), intent(out) :: xf
  ...
  real, dimension(mkxcrt), parameter :: pcrit = (/ 850., 837.5, 825.,   &
       812.5, 800., 787.5, &
       775., 762.5, 750., 737.5, 725., 712.5, &
       700., 687.5, 675., 662.5, 650., 637.5, &
       625., 612.5, 600., 550.,  500., 450., &
       400. /)
  real, dimension(mkxcrt), parameter :: acrit = (/6.323E-02,    &
       5.795E-02, 5.390E-02,  5.236E-02, &
       ...
       0.368/)
  real, dimension(mkxcrt), parameter ::  acritt = (/0.203,    &
       0.299, 0.359,  0.403, 0.515,  0.478, &
...
subroutine cup_output_ens_shal(k2, xf, ierr, dellat, dellaq,     &
  ...
  integer, intent(in) :: maxens2
  integer, intent(in) :: ensdim
  real, dimension(ensdim), intent(in) :: xf
