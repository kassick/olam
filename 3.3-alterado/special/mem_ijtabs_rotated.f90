...
Module mem_ijtabs
   ...
   use max_dims, only: maxgrds         ! max number of nested grid areas

   integer, parameter :: nloops_m =  4 ! max number of DO loops for M pts
   integer, parameter :: nloops_u = 35 ! max number of DO loops for U pts
   integer, parameter :: nloops_w = 45 ! max number of DO loops for W pts
   integer, parameter :: maxtpn   =  8 ! max number (+1) of U/W pts around M pt 
   ...
   Type itab_u_vars             ! data structure for U pts (individual rank)
      ...
      real :: vzu1_u=0., vzu2_u=0., vzu3_u=0., vzu4_u=0., vzw1_u=0., vzw2_u=0. ! proj coefs
   ...
   Type itab_w_vars                   ! data structure for W pts (individual rank)
      ...
      integer :: mrlw=0, mrlw_orig=0, mrow=0 ! mesh refinement level of this W pt
   ...
   type (itabg_m_vars), allocatable :: itabg_m(:)
   type (itabg_u_vars), allocatable :: itabg_u(:)
   type (itabg_w_vars), allocatable :: itabg_w(:)
...
!===============================================================================
   ...
   return
   end subroutine alloc_jtabs1

!===============================================================================

   subroutine alloc_jtabs2()
...
!------------------------------------------------------------------------
   if (allocated(itab_m)) call vtables(ndims,idims  &
      ,'IMGLOBE   :hist:nohist'  ,ivara1=itab_m(:)%imglobe)
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
   if (allocated(itab_u)) call vtables(ndims,idims  &
      ,'IUGLOBE   :hist:nohist'  ,ivara1=itab_u(:)%iuglobe)
!------------------------------------------------------------------------
   if (allocated(itab_u)) call vtables(ndims,idims  &
      ,'IRANKU    :hist'         ,ivara1=itab_u(:)%irank)
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
   if (allocated(itab_w)) call vtables(ndims,idims  &
      ,'IWGLOBE   :hist:nohist'  ,ivara1=itab_w(:)%iwglobe)
!------------------------------------------------------------------------
   if (allocated(itab_w)) call vtables(ndims,idims  &
      ,'IRANKW    :hist'         ,ivara1=itab_w(:)%irank)
!------------------------------------------------------------------------
