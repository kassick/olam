...
use misc_coms,  only: io6, dtlm, cnum_sclr, time8
use mem_turb,   only: vkh, hkm, sxfer_rk
use mem_grid,   only: mza, mua, mwa, lpw, lsw,  &
                      dniu, volt, aru, arw, volwi, dzim, volti, zm, dzt
use consts_coms, only: grav, pi4, rdry, pi1, pi2, pio2
...
!-------------------------------------
! Only for ncar test cases:
real :: tau,omeg0,temp0,scale_height,pbot,ptop,patw,sp,wcase3,wmarwsc_b
!-------------------------------------
...
! Only for ncar test cases:
   if (ncar_testcase == 3) then
      ...
      omeg0 = pi4 * 1.e4 / tau
      temp0 = 300.
      scale_height = rdry * temp0 / grav
      pbot = 1.e5
      ptop = pbot * exp(-zm(mza-1) / scale_height)
      patw = pbot * exp(-zm(k)     / scale_height)
   
      sp = min(1.,2. * sqrt(sin((patw - ptop) / (pbot - ptop) * pi1)))

      wcase3 = -scale_height / patw  &
             * omeg0 * cos(pi2 * time8 / tau) * sin(sp * pio2) 
   
!      wmarwsc(k,iw) = arw(k,iw) * wcase3 * dzim(k)  &
!                * .5 * (dzt(k+1) * rho(k,iw) + dzt(k) * rho(k+1,iw))
                
      wmarwsc(k,iw) = arw(k,iw) * wcase3 * patw / (rdry * temp0)          
                
!     if (iw == 1000) then
!        print*, 'wcase3 ',k,wcase3,sp,patw,omeg0,scale_height,  &
!           wmarwsc(k,iw),wmarwsc_b  
!     endif
