...
real fu_sr(11:13)	!TOA MODEL WINDOW RADIANCEs
real fu_sf(11:13)	!TOA MODEL WINDOW FLUXs
real trwn_flt_r		!emmulated Flt WN Radiance @TOA ( W  m-2 sr-1)
real trwn_unf_r		!emmulated Unflt WN Radiance @TOA ( W  m-2 sr-1)
real trwn_f		!emmulated WN Flux @TOA ( W  m-2 sr-1)
...
real rswfu(nv1x,15)	!SW (0.2-4.0) Up
real rswfd(nv1x,15)	!SW (0.2-4.0) Dn
real rswdir(nv1x,15)	!SW (0.2-4.0) Direct
real rswdif(nv1x,15)	!SW (0.2-4.0) Diffuse

real rlwfu(nv1x,7:20)	!LW (4.0-->) Up
real rlwfd(nv1x,7:20)	!LW (4.0-->) Dn
real sbf(nv1x,7:20)	!Level Sig*T**4 (4.0-->)
real sbs(7:20)		!SFC Sig*T**4 (4.0-->)
real swalbtoa(15)
real swalbsfc(15)
real swtran(15)   
real dirsfc(15)  !! CORRECTED FOR FU ICE TAU
real difsfc(15)  !! CORRECTED FOR FU ICE TAU
logical exist		! Flag to test for presence 
...
subroutine only_toa_sfc
  ...
  do i=1,nscm
	...
	enddo
        ...
        fsfc(i)%dirsfc =   sum( fos(i)%dirsfc(1:15) )
        fsfc(i)%difsfc =   sum( fos(i)%difsfc(1:15) )
     endif
  enddo
...
!------------------------------------------------------------------
subroutine flx_cloudcombine(icmf,icp,iw)
implicit none
