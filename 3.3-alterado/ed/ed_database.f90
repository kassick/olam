...
!  character(len=256), parameter :: ed_fprefix = '/cluster/home/dmm31/ed_inputs/amazon_restart/lu.sa.05.restart.01.'
  ...
  real :: flat
  real :: flon
  character(len=256) :: pss_name
  character(len=256) :: css_name
  ...
  site_loop: do while(associated(cs))
     ...
     if(edres == 1.0)then
        ...
        elseif(flat < 0.0 .or. flat >= 10.0)then
           ...
           write(ed_fname,'(a,f3.1)')trim(ed_hfilin)//'lat',flat
        endif
        if(flon <= -100.0)then
     ...
     if(.not.restart_exist)then
        ...
        if(associated(cs,target=land%first_site))then
           land%first_site => cs%next_site
        else
           tcs => land%first_site
