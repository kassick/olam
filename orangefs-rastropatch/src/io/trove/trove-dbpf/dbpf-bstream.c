...
inline int dbpf_pwrite(int fd, const void *buf, size_t count, off_t offset)
{
    ...
    rst_event_l(DBPF_WRITE_OUT,fd);

    if(ret < 0)
    {
        return -trove_errno_to_trove_error(errno);
    }
     //rastro: instrumentar aqui para o FD e o ret_size (int,int)
#ifdef HAVE_RASTRO
     //rst_event_ii(1,fd,ret_size);
#endif
