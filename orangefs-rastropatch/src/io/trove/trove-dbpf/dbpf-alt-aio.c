                   ...
                   int nent, struct sigevent *sig) 
{
    ...
    cur_op = (dbpf_queued_op_t *)sig->sigev_value.sival_ptr;
    //   from op_p->u.b_rw_list.sigev.sigev_value.sival_ptr = (void *)q_op_p;
    ...
    if (cur_op->op.u.b_rw_list.opcode == LIO_READ)
        rst_event_ll(DBPF_READ_IN, cur_op->op.short_id,   cur_op->op.u.b_rw_list.fd);
    else
        rst_event_ll(DBPF_WRITE_IN, cur_op->op.short_id,   cur_op->op.u.b_rw_list.fd);
#endif
...
static void* alt_lio_thread(void* foo)
{
    ...
    else if(tmp_item->cb_p->aio_lio_opcode == LIO_WRITE)
    {
        ...
        //rst_event_l(DBPF_WRITE_IN,tmp_item->cb_p->aio_fildes);

        ret = pwrite(tmp_item->cb_p->aio_fildes,
                     (const void*)tmp_item->cb_p->aio_buf,
                     tmp_item->cb_p->aio_nbytes,
                     tmp_item->cb_p->aio_offset);
        //rst_event_l(DBPF_WRITE_OUT,tmp_item->cb_p->aio_fildes);
    }
    else
    {
