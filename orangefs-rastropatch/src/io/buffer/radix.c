...
/* new functions */
void radix_rst_init(rst_t *t, unsigned long (* get_value)(const void *), int max_b)
{
    rst_node_t *r;
...
static void radix_rst_finalize(rst_t *t) __attribute__((unused));
static void radix_rst_finalize(rst_t *t) 
{
    rst_free_dfs(t->root);
}
